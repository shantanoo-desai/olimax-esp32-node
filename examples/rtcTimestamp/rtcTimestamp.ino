/*  This Sketch provides Epoch Timestamps for every measurement that is sent to
 *  InfluxDB via RTC D3231. The RTC is connected to the Olimex ESP32-PoE board
 *  via I2C bus.
 *  NOTE: Please refer to the Hardware Schematics of the board in order to interface
 *        the RTC to I2C_SDA and I2C_SCL resp.
 *  Author: Shantanoo Desai   <des@biba.uni-bremen.de>
 */

 #define ETH_CLK_MODE ETH_CLOCK_GPIO17_OUT
 #define ETH_PHY_POWER 12

/* I2C GPIO Pins for Olimex ESP32-PoE Boards */
#define I2C_SDA  13
#define I2C_SCL  16

 #include <ETH.h>
 #include <Wire.h> // for I2C
 #include <RTClib.h> // for RTC DS3231
 #include "UMG_Node.h"

 /* STATIC IP CONFIGURATION for the Node */
 /* NOTE: not necessary if you have DHCP on your network and know
         the IP address of InfluxDB
 */
 const IPAddress node_static_ip(192, 168, 3, 101);
 const IPAddress default_gateway(192, 168, 3, 250);
 const IPAddress subnet_mask(255, 255, 255, 0);
 const IPAddress dns_address(192, 168, 3, 250);

 /* InfluxDB CONFIGURATION */

 const char* INFLUXDB_IP = "192.168.3.11"; // add your InfluxDB's IP address here
 const uint16_t INFLUX_PORT = 8086;
 const char* INFLUX_DB_NAME = "example"; // add the name of the Database

 // create databse for the above configuration
 InfluxDB db(INFLUXDB_IP, INFLUX_PORT, INFLUX_DB_NAME);

/* RTC DS3231 */
RTC_DS3231 rtc;

long randomNumber;

// ethernet connectivity bool flag
static bool eth_connected = false;

// Event Handler for Ethernet Port
void EthEvent(WiFiEvent_t event)
{
  switch (event) {
   case SYSTEM_EVENT_ETH_START:
     Serial.println("ETH Started");
     //set eth hostname here
     ETH.setHostname("esp32-ethernet");
     break;
   case SYSTEM_EVENT_ETH_CONNECTED:
     Serial.println("ETH Connected");
     break;
   case SYSTEM_EVENT_ETH_GOT_IP:
     Serial.print("ETH MAC: ");
     Serial.print(ETH.macAddress());
     Serial.print(", IPv4: ");
     Serial.print(ETH.localIP());
     if (ETH.fullDuplex()) {
       Serial.print(", FULL_DUPLEX");
     }
     Serial.print(", ");
     Serial.print(ETH.linkSpeed());
     Serial.println("Mbps");
     eth_connected = true;
     break;
   case SYSTEM_EVENT_ETH_DISCONNECTED:
     Serial.println("ETH Disconnected");
     eth_connected = false;
     break;
   case SYSTEM_EVENT_ETH_STOP:
     Serial.println("ETH Stopped");
     eth_connected = false;
     break;
   default:
     break;
  }
}

void setup()
{
 // Enable I2C with 400 kHz
 Wire.begin(I2C_SDA, I2C_SCL, 400000);
 Serial.begin(115200);
 WiFi.onEvent(EthEvent);

 // Check RTC is connected on I2C bus or not.
 if (!rtc.begin()) {
   Serial.println("No RTC detected on I2C!");
   while (1); // stop here...
 }

 // ------------>>>>> SET UTC TIME HERE <<<<<---------------- //
 // Always try setting the RTC Time to UTC in order to avoid conflict in
 // Timezones when visualizing data
 // FORMAT: YYYY.MM.DD. hh.mm.ss
 rtc.adjust(DateTime(2018, 10, 10, 14, 0, 0 ));

 ETH.begin();
 // Enable STATIC IP Address for the Node
 // NOTE: not necessary if DHCP used.
 ETH.config(node_static_ip, default_gateway, subnet_mask, dns_address);
 delay(1000);
}

void loop()
{
  if (eth_connected) {
   DateTime current_time = rtc.now();
   randomNumber = random(10, 20);
   influxMeasurement row("random"); // Name of the Measurement
   row.addTag("name", "node1"); // add tag for your measurement
   row.addTag("loc", "lab");
   row.addField("value", randomNumber); // add field name and value
   row.addTimeStamp(current_time.unixtime()); // Unix Timestamps are always in 'seconds' precision

   // Serial.println(row.postString());// DEBUG: See how your Line Protocol Looks like..

   // Send it via HTTP
   // do not forget to mention the precision parameter in `httpWrite`.
   // here it is seconds -> "s"
   Serial.println(db.httpWrite(row, "s") == INFLUX_SUCCESS ? "Success Write" : "Failed Write" );
  }
  delay(1000);
}
